...
checkout = (idTask, persoInfo, cardInfo, tasks) => {
  let task = JSON.parse(localStorage.AllTasks);
  task = task[idTask];
  updateTab(task.tabId, "https://www.supremenewyork.com/checkout", () => {
    chrome.tabs.executeScript(task.tabId, {
      code: "checkout(" + idTask + "," + JSON.stringify(persoInfo) + "," + JSON.stringify(cardInfo) + "," + JSON.stringify(tasks) + ")",
    });
  });
};
  ...
  while (i < words.length) {
    ...
    if (finalInsert != null || finalInsert != undefined) {
      ...
      if (AllTasksParse[taskNb].stateTimerBtn == "true" && AllTasksParse[taskNb].timer != null && AllTasksParse[taskNb].state == 1 && nbRefresh <= 40) {
        nbRefresh++;
        setTimeout(function () {
          refindLink(cats, keywords, taskNb, nbRefresh);
        }, 500);
      } else {
...
function successCheckout(){
  ...
  while (i < AllTasksParse.length) {
    if (AllTasksParse[i].status == "On Play") {
      if (AllTasksParse[i].state == "1") {
        AllTasksParse[i].state = "0";
        AllTasksParse[i].status = "Successfully";
        notificationsSend('successNotif', `Successfully execution task n°${i}`, `Well done !`)
      }
      changeStorageValue(AllTasksParse);
    }
    i++;
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  let task;
  if (request.msg != "startCopTimer" && request.msg != "successCop") {
  ...
  switch (request.msg) {
    case "addItemToBasket":
      if (task.length - 1 == request.idtaskitem && AllTasksParse[request.idtask].addToCart == "false") {
        ...
        checkout(request.idtask, localStorage.persoInfo, localStorage.cardInfo, localStorage.AllTasks);
      } else {
